...
mongoose.connect('mongodb+srv://recep:bjk1903arda@backupbotu.p1zc8.mongodb.net/backupbotu?retryWrites=true&w=majority', {useNewUrlParser: true, useUnifiedTopology: true});// Mongo connect bağlantısı.
const Database = require("./models/role.js");


client.on("ready", async () => {
  client.user.setPresence({ activity: { name: "ASPENDOS ❤️ PUSHA" }, status: "idle" });
...
function cezalandir(kisiID, tur) {
  ...
  if (tur == "ban") return uye.ban({ reason: "Pusha Koruma" }).catch();
};
...
client.on("roleCreate", async role => {
  ...
  if (!entry || !entry.executor || Date.now()-entry.createdTimestamp > 5000 || guvenli(entry.executor.id) || !ayarlar.roleGuard) return;
  role.delete({ reason: "Pusha Rol Koruma" });
